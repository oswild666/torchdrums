...
class Instrument:
    ...
    def play(self, temp_params=None):
        ...
        # Use temporary, modulated parameters if provided by the synth engine
        if temp_params:
...
class KickReso(Instrument):
    ...
    def play(self):
        ...
        # Input signal: a short pulse
        impulse = np.zeros(length_samples)
        impulse[0] = 1.0

        # Frequency envelope for the filter cutoff
        freq_env = np.linspace(self.params['start_freq'], self.params['end_freq'], length_samples)

        # Apply filter with sweeping cutoff - this is slow in Python, a real implementation would be C++
        output = np.zeros(length_samples)
        z = np.zeros(2) # Filter state
        q = self.params['Q']
...
class Synth:
    ...
    def trigger(self, instrument_name):
        with self.lock:
...
# --- Audio Thread (unchanged from previous step) ---
    ...
    def run(self):
        try:
            self.stream = sd.OutputStream(
                samplerate=SAMPLE_RATE, blocksize=BUFFER_SIZE, channels=2,
...
class ModulationCurveWidget(QGraphicsView):
    ...
    def draw_at(self, pos):
        ...
        if 0 <= index < len(self.curve_data):
            # Linear interpolation for smoother drawing
...
class ModulationEditor(QGroupBox):
    ...
    def __init__(self, parent=None):
        ...
        self.layout.addWidget(self.curve_view, 5) # 5/6 of the space

        controls_layout = QVBoxLayout()

        self.add_control_widgets(controls_layout)

        self.layout.addLayout(controls_layout, 1) # 1/6 of the space
        self.setLayout(self.layout)
        self.hide()

    def add_control_widgets(self, layout):
        self.depth_slider = QSlider(Qt.Vertical)
        self.depth_slider.setRange(0, 100)
        self.depth_slider.valueChanged.connect(self.update_mod_data)
        layout.addWidget(QLabel("Depth"))
        layout.addWidget(self.depth_slider, alignment=Qt.AlignCenter)

        self.polarity_combo = QComboBox()
        self.polarity_combo.addItems(["Unipolar", "Bipolar"])
        self.polarity_combo.currentIndexChanged.connect(self.update_mod_data)
        layout.addWidget(QLabel("Polarity"))
        layout.addWidget(self.polarity_combo)

        self.sync_combo = QComboBox()
        self.sync_combo.addItems(["Global", "Track"])
        self.sync_combo.currentIndexChanged.connect(self.update_mod_data)
        layout.addWidget(QLabel("Sync"))
        layout.addWidget(self.sync_combo)

        clear_btn = QPushButton("Clear")
        clear_btn.clicked.connect(self.clear_curve)
        layout.addWidget(clear_btn)

        random_btn = QPushButton("Random")
        random_btn.clicked.connect(self.randomize_curve)
    ...
    def clear_curve(self):
        self.curve_view.set_curve_data(np.full(MAX_STEPS, 0.5))
        self.update_mod_data()

    def randomize_curve(self):
        self.curve_view.set_curve_data(np.random.rand(MAX_STEPS))
        self.update_mod_data()

class ModdableSlider(QWidget):
    modulation_requested = Signal(object, str) # instrument, param_name
    ...
    def __init__(self, instrument, param_name, param_def):
        ...
        self.label = QLabel(param_name.replace('_', ' ').title())
        self.label.setFixedWidth(100)

        self.slider = QSlider(Qt.Horizontal)
        self.slider.setRange(param_def['min'], param_def['max'])
        scale = param_def.get('scale', 1.0)
        self.slider.setValue(int(instrument.params[param_name] * scale))
        self.slider.valueChanged.connect(self.slider_changed)

        self.mod_badge = QLabel("MOD")
        self.mod_badge.setFixedWidth(30)
        self.mod_badge.setStyleSheet("color: #007ACC; font-weight: bold;")
        self.mod_badge.hide()

        layout.addWidget(self.label)
        layout.addWidget(self.slider)
        layout.addWidget(self.mod_badge)
        self.setLayout(layout)

        self.update_mod_badge()
    ...
    def mousePressEvent(self, event):
        self.modulation_requested.emit(self.instrument, self.param_name)
...
class SequencerTrack(QWidget):
    def __init__(self, name, steps=MAX_STEPS):
        ...
        for i in range(self.num_steps):
            ...
            step_button.setFixedSize(25, 25)
            layout.addWidget(step_button)
            self.steps.append(step_button)

        self.length_spinbox = QSpinBox()
        self.length_spinbox.setRange(1, MAX_STEPS)
        self.length_spinbox.setValue(MAX_STEPS)
        self.length_spinbox.setFixedWidth(50)
    ...
    def update_playback_position(self, global_step):
        ...
        if self.track_length > 0:
            ...
            if local_step < len(self.steps):
                ...
                new_style = current_style + "QPushButton { border: 2px solid cyan; }"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 1200, 950) # Increased height
        ...
        # --- Instrument Controls Area with Scroll ---
        scroll_area = QScrollArea()
        scroll_area.setWidgetResizable(True)
        scroll_area.setFixedHeight(250) # Fixed height for the scroll area

        scroll_widget = QWidget()
        self.instrument_controls_layout = QHBoxLayout(scroll_widget)

        scroll_area.setWidget(scroll_widget)
        self.main_layout.addWidget(scroll_area)

        self.instrument_controls_widgets = {} # To hold references to ModdableSliders
        ...
        self.mod_mode_button = QPushButton("+ Modulation Mode")
        self.mod_mode_button.setCheckable(True)
        self.mod_mode_button.toggled.connect(self.on_mod_mode_toggled)
        mod_controls_layout.addWidget(self.mod_mode_button)
        mod_controls_layout.addStretch()
    ...
    def create_sequencer_ui(self):
        ...
        for name, instrument in self.synth.instruments.items():
            track = SequencerTrack(instrument.name)
            self.tracks[name] = track
            sequencer_layout.addWidget(track)

        sequencer_group.setLayout(sequencer_layout)
        self.main_layout.addWidget(sequencer_group)
    ...
    def create_instrument_controls(self):
        for name, instrument in self.synth.instruments.items():
            ...
            for param_name, p_def in instrument.param_defs.items():
                ...
                # Create a container for the slider and its mod button
                control_widget = ModdableSlider(instrument, param_name, p_def)

                # Connect the request signal to the main window's handler
    ...
    def on_modulation_requested(self, instrument, param_name):
        """Handle clicks on sliders when in modulation mode."""
        if self.mod_mode_button.isChecked():
            self.mod_editor.set_target(instrument, param_name)
            self.mod_editor.show()
    ...
    def on_mod_mode_toggled(self, checked):
        if checked:
            ...
            self.mod_editor.hide() # Hide until a slider is clicked
        else:
            self.mod_mode_button.setStyleSheet("") # Revert to default
...
if __name__ == "__main__":
    ...
    if "Chicago" in font_db.families():
        app.setFont(QFont("Chicago", 12))
    ...
    app.setStyleSheet("""
        ...
        SequencerTrack QPushButton {
             border-radius: 4px;
        }
        SequencerTrack QPushButton:checked {
            background-color: #D2691E; /* A reddish/orange accent */
